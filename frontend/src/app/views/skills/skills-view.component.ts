...
@Component({
  ...
  template: `
    <section class="skills section parallax-section parallax-skills" id="skills" data-parallax data-parallax-speed="0.12" data-parallax-speed2="0.24">
      <app-section-header 
        title="Skills" 
        subtitle="Tools I reach for before the coffee cools"
        number="04">
      </app-section-header>
      ...
      @if (!categories.length) {
        <div class="empty-state reveal-on-scroll">
          ...
          <p class="empty-copy">Promise I know things — just need to name them.</p>
        </div>
      }

      <div class="skills-grid">
        @for (category of categories; track category.id; let i = $index) {
          <div class="skill-category reveal-on-scroll hover-elevate" [class]="getCategoryClass(i)">
...
export class SkillsViewComponent {
  ...
  getCategoryClass(index: number): string {
    const classes = ['category-violet', 'category-pink', 'category-orange', 'category-mango'];
